...
#Setting up list inputs for custom dataframe and assumptions
...
def WAL_Consolidator(df):
    ...
    for i in range(0,df.shape[0]):
        ...
        wal_array[i] = (calculate_WAL(principal,rate_array[i],term_array[i],speed_array[i],adjustment_input,abs_value))/12
        counter += 1

    my_dict_array = {'Security':security_array,'Term': term_array, 
                     'Yield':rate_array,'Speed':speed_array
                     ,'WAL':wal_array}
    new_df = pd.DataFrame(my_dict_array)
    print(new_df)

    return new_df, counter

dataset, iterations = WAL_Consolidator(df)
time_elapse = time.time() - start_time

dataset.to_csv('Portfolio Speed & Weighted Average Life.csv',index=False)
print(f'The script ran a total of {iterations} amortization iterations and took {round(time_elapse,2)} seconds to run')

profile = ProfileReport(dataset, title='Speed & Weighted Average Life Analysis')
